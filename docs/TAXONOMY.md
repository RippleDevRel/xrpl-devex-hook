...
- `agent`: the coding agent that produced the row (`claude-code`, `codex`, `cursor`, `grok`, `vscode-copilot`), null when the hook could not tell. Not personal data.
- `tx_hash`: the first validated transaction hash printed next to a captured result code, uppercase hex, null otherwise. Public, kept so a hook row can be verified on the ledger; `text` never carries hashes.
- `author`: `agent | participant`, null on hook rows. Reflections are written by the agent on the participant's behalf (`agent`); `/xrpl-feedback` rows and session analysis items come from the participant (`participant`). Recaps and scoring weigh the two apart.
